...
/* USER CODE BEGIN 0 */
	...
	void TIM2_IRQHandler(void) {
		if ((TIM2->SR & 0x0002)!=0) { // Si la comparacion es exitosa, la IRQ se lanza
		// y la ISR se ejecuta. Esta linea chequea si la linea lanza el ISR
			GPIOC->BSRR |= (1 << (8 + 16));
			TIM2->CNT = 0;
			TIM2->SR = 0x0000; // Limpio Flags
			TIM2->CR1 = 0x0000;
		}
	}
	void TIM3_IRQHandler(void) {
			...
			// y la ISR se ejecuta. Esta linea chequea si la linea lanza el ISR
				if(flanco == 0){
					tiempo_echo = TIM3->CCR4;
					flanco++;
				}else{
					tiempo = TIM3->CCR4 - tiempo_echo;
					flanco = 0;
				}
				if(tiempo < 0) tiempo += 0xFFFF;
				TIM3->SR = 0x0000; // Limpio Flags
			}
		}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  // Configuracion PA5 LED
  	...
  	TIM2->CR1 = 0x0000; // ARPE = 0 -> No PWM, es TOC
  	 // CEN = 0; Contador Apagado
  	TIM2->CR2 = 0x0000; // Siempre "0"
  	TIM2->SMCR = 0x0000; // Siempre "0"
  	// Config. comportamiento contador: PSC, CNT, ARR y CCRx
  	TIM2->PSC = 32 - 1; // Preescalado=32000 -> F_counter=32000000/32000 = 1000 pasos/segundo
  	TIM2->CNT = 0; // Inicio el contador a 0
  	TIM2->ARR = 0xFFFF; // Valor Recomendado = FFFF
  	TIM2->CCR1 = 15; // Record that stores the value for the comparison.
  	 // It is initialized to the first value = 500 steps = 0.5 seconds
  	// Seleccionando o no IRQ: DIER
  	TIM2->DIER = 0x0002; // Hay IRQ despues de comparacion exitosa -> CCyIE = 1
  	// Modo Salida
  	TIM2->CCMR1 = 0x0000; // CCyS = 0 (TOC)
  	 // OCyM = 000 (no hay salida hardware)
  	 // OCyPE = 0 (no hay preload)
  	TIM2->CCER = 0x0000; // CCyP = 0 (siempre en TOC)
  	 // CCyE = 0 (desactivada salida hardware)
  	// Inicializando contador
  	TIM2->EGR |= 0x0001; // UG = 1 -> Genero un evento de actualizacion
  					// para actualizar los registros
  	TIM2->SR = 0; // Limpio flags
  	// Enabling TIM2_IRQ at NVIC (position 28).
  	NVIC->ISER[0] |= (1 << 28);
	...
	TIM3->CR1 = 0x0000; // ARPE = 0 -> No PWM, es Tic
	 // CEN = 0; Contador OFF
  	...
  	TIM3->ARR = 0xFFFF;
  	TIM3->CCR4 = 0; // Se almacena el valor contado (en TIC no hae falta instanciarlo)
  	// Seleccion de IRQ: DIER
  	TIM3->DIER = 0x0010; // Hay IRQ cuando la cuenta ha terminado -> CC4IE = 1
  	// Modo output
  	TIM3->CCMR2 = 0x0100; // CC1S = 01 (TIC)
  	TIM3->CCER = 0xB000; // CC1P = 1, CC1NP = 1 (por ambos flancos)
  	 	 	 	 	 	 // CC1E = 1 (Salida hardware activada)
  	// Encendiendo contador
  	TIM3->CR1 |= 0x0001;	 // CEN = 1 -> Empezar Contador
	TIM3->EGR |= 0x0001; // UG = 1 -> Genero un evento de actualizacion
					// para actualizar los registros
	TIM3->SR = 0; // Limpio flags
	// Enabling TIM3_IRQ at NVIC (position 29).
	NVIC->ISER[0] |= (1 << 29);
  /* USER CODE END 2 */
	  	  ...
	  	  if(medida != medida_act){
	  		  medida_act = medida;
	  			  if(modo_config == 0 && medida_act == 1){
	  				  ...
	  				  //angulo = (((uint8_t*)&texto[0])* 180) / 4095; // Cálculo del angulo
	  				  // Ensenho el resultado en Tera Term
	  				  printf((uint8_t*)texto);
	  				  printf("\n\r");
	  				  tiempo_actual = tiempo;
	  				printf("%i",tiempo_actual); // Comprobar tiemop correcto
					  printf("\n\r");
				  ...
				  medida = 0;
	  				...
	  				tiempo_actual = tiempo;
					printf("%i",tiempo_actual); // Comprobar tiempo correcto
					  printf("\n\r");
	  				distancia = tiempo / 58; // Calcular distancia medida con ultrasonidos
				  printf("%i",distancia);
				  printf("\n\r");
