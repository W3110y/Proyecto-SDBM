...
/* USER CODE BEGIN 0 */
	...
	// Función  para sustituir a printf()
	void UART_Mensaje(char *msg) {
	    uint16_t longitud = 0;

	    // Contamos cuántas letras hay hasta encontrar el '\0'
	    while (msg[longitud] != '\0') {
	        longitud++;
	    }

	    // Enviamos el mensaje por la UART
	    HAL_UART_Transmit(&huart2, (uint8_t*)msg, longitud, 100);
	}
/* USER CODE END 0 */
  ...
  while (1)
  {
	  ...
	  if(dato_listo == 1){
		  if(modo_config == 0){
			  ...
			  UART_Mensaje("Modo Manual -> distancia: \n\r");
			  Bin2Ascii(distancia, &texto[0]);
			  HAL_UART_Transmit(&huart2, (uint8_t*)texto, 6, 100);
			  UART_Mensaje(" cm\n\r");
			  ...
			  if(giros == 4){
				UART_Mensaje("Modo Automatico -> distancia: [");
				for(int i = 0; i < 5; i++) {
					Bin2Ascii(distancias[i], &texto[0]);
					HAL_UART_Transmit(&huart2, (uint8_t*)texto, 6, 100);
					if(i != 5) UART_Mensaje(", ");
				}
				UART_Mensaje("] cm\r\n");
